$(document).ready(function() {
     ...
     */
    // Fetch all to apply validation styles to
        ...
        if (total_element < max) {
            ...
            $("#addavail_" + next_index)
                .append(
                    `<div class="row">
                        <div class="col-sm-9">
                            <input type="text" class="form-control avail_picker form-valid" name="availability_${next_index}" />
                        </div>
                        <div class="col-sm-3 my-auto">
                            <span id="removeavail_${next_index}" class="remove_avail_button">Remove</span>
                        </div>
                    </div>`
                )
                .find(".avail_picker")
    ...
    $(".participant_add").on("click", function() {
        ...
        for (let i = 1; i <= element_avail_num; i++) {
            let html_isert_td =
                `<td>
                    <select class="select_avail" name="part_${next_index}_avail_${i.toString()}" form="form1">
        ...
        $("#participant_" + next_index).append(
            `<td>
                <input type="text" class="form-control" name="participant_${next_index}" form="form1" required/>
             </td>
                ${html_isert_td_all}
             <td>
             ...
             <td>
                 <button id ="removeparticipant_${next_index}" class="participant_remove">Remove</button>
             </td>
             <td></td>`
        );
    });
    ...
    $(".button_edit").on("click", function() {
        ...
        let element_avail_num = document.getElementsByClassName("avail").length;
        let html_insert_td_all = "";
        for (let i = 1; i <= element_avail_num; i++) {
            html_insert_td_all +=
                `<td>
                    <select class="select_avail" form="form2" name="availability_${i.toString()}">
                        <option value="Yes">Yes</option>
                        <option value="No">No</option>
                        <option value="Maybe">Maybe</option>
                    </select>
        ...
        let insert_html =
            `<tr>
                <td>
                    <input type="text" class="form-control" name="edit_name" value="${name_clicked}" form="form3" readonly/>
                    <input type="hidden" name="edit_name" value="${name_clicked}" form="form2" readonly/>
                </td>
                    ${html_insert_td_all}
                <td>
    ...
    for (let i = 0; i < element_avail_num; i++) {
        ...
        $(test_variable).append(
            `<div>Yes - ${num_yes}</div><div>No - ${num_no}</div><div>Maybe - ${num_maybe}</div>`
        );
    }
});
